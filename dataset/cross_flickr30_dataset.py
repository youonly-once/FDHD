...
def load_dataset(dataset, batch_size,transform):
    ...
    train_loc = 'F:/datasets/' + dataset + '/train_l_uni_Np.pkl'
    query_loc = 'F:/datasets/' + dataset + '/query_l_uni_Np.pkl'
    retrieval_loc = 'F:/datasets/' + dataset + '/retrieval_l_uni_Np.pkl'
   ...
   # raw_images = {'train': train_raw_images[:4992], 'query': query_raw_images, 'retrieval': retrieval_raw_images}
    text_token = {'train': train_text_token[:4992], 'query': query_text_token, 'retrieval': retrieval_text_token}
    imageu = {'train': train_image_u[:4992], 'query': query_image_u, 'retrieval': retrieval_image_u}
    text_raw = {'train': train_texts_raw[:4992], 'query': query_texts_raw, 'retrieval': retrieval_texts_raw}
    labels= {'train': train_labels[:4992], 'query': query_labels, 'retrieval': retrieval_labels}
    image_path= {'train': train_image_path[:4992], 'query': query_image_path, 'retrieval': retrieval_image_path}
    ...
    pin = {'train': True, 'query': False, 'retrieval': False}
    dataloader = {x: MultiEpochsDataLoader(dataset[x], batch_size=batch_size, drop_last=drop_last[x], pin_memory=pin[x], shuffle=shuffle[x],
                                num_workers=6) for x in ['train', 'query', 'retrieval']}
