...
class CustomDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        end_time = time.time()
        return raw_image, text_token, imgu, self.txtsent[index], index,lab
...
def load_dataset(dataset, batch_size,transform):
    ...
    with open(train_loc, 'rb') as f_pkl:
        ...
        train_txtsent = torch.tensor(data['caption_features'] ,dtype=torch.float32)
    ...
    with open(query_loc, 'rb') as f_pkl:
        ...
        query_txtsent = torch.tensor(data['caption_features'] ,dtype=torch.float32)
    ...
    with open(retrieval_loc, 'rb') as f_pkl:
        ...
        retrieval_txtsent = torch.tensor(data['caption_features'] ,dtype=torch.float32)
    ...
    raw_images = {'train': train_raw_images, 'query': query_raw_images, 'retrieval': retrieval_raw_images}
    txtsent = {'train': train_txtsent, 'query': query_txtsent, 'retrieval': retrieval_txtsent}
    ...
    dataset = {x: CustomDataSet(raw_images=raw_images[x],
                                ...
                                imageu=imageu[x], txtsent=txtsent[x],text_raw=text_raw[x],
